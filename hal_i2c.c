...
#define I2C_DISABLE() st( I2CCFG &= ~(I2C_ENSI); )
...
i2cLen_t HalI2CRead(i2cLen_t len, uint8 *pBuf)
{
 ...
 // Added to make sure all bytes are ACK'd except for the last one which is NACK'd. If only
 // 1 byte is being read, a single NACK will be sent. 
  if (len > 1)
  {
    I2C_SET_ACK();
  }
 ...
 */
i2cLen_t HalI2CWrite(i2cLen_t len, uint8 *pBuf, bool stp)
{
